...
///
/// `#[rustfmt::skip]` keeps the column-aligned tuple layout — the
/// migration script (`tools/migrate-premium-paths.ts:emitRustTable`)
/// emits aligned output and `committed_endpoint_tiers_rs_exactly_matches_script_output`
/// pins the file to that exact form. Without the skip, rustfmt collapses
/// the alignment and the bun test fails on every push.
#[rustfmt::skip]
