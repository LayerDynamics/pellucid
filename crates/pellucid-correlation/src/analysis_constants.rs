...
pub static STOP_WORDS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    [
        ...
        "than", "too", "very", "just", "also", "now", "new", "says", "said", "after",
    ]
    .into_iter()
    .collect()
});
...
pub static TOPIC_MAPPINGS: Lazy<BTreeMap<&'static str, &'static [&'static str]>> =
    Lazy::new(|| {
...
mod tests {
    ...
    fn jaccard_disjoint_sets_score_zero() {
        let a: HashSet<String> = ["iran", "missile"]
            .iter()
            .map(|s| (*s).to_string())
            .collect();
        let b: HashSet<String> = ["stock", "rally"]
            .iter()
            .map(|s| (*s).to_string())
            .collect();
        assert_eq!(jaccard_similarity(&a, &b), 0.0);
