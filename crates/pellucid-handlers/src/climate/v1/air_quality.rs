...
mod tests {
    ...
    async fn returns_503_when_cache_empty() {
        ...
        let resp = app
            .oneshot(Request::builder().uri(AIR_QUALITY_PATH).body(Body::empty()).unwrap())
            .await
            .unwrap();
    ...
    async fn computes_worst_aqi() {
        ...
        let resp = app.oneshot(Request::builder().uri(AIR_QUALITY_PATH).body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = axum::body::to_bytes(resp.into_body(), 1_000_000).await.unwrap();
        let parsed: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(parsed.pointer("/worstAqi").and_then(Value::as_u64), Some(250));
    }
}
