//! `GET /api/climate/v1/earthquakes` — pure reader for USGS recent
//! quakes.

use axum::extract::State;
use axum::Json;
use serde::{Deserialize, Serialize};
use serde_json::Value;

use pellucid_cache::get_cached_json;

use crate::economic::v1::shared::{decode_required, HandlerError};
use crate::state::AppState;

pub const CACHE_KEY: &str = "seismology:recent-quakes:24h:v1";

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct QuakeRow {
    pub id: String,
    pub place: String,
    pub mag: f64,
    pub depth: f64,
    pub lat: f64,
    pub lon: f64,
    #[serde(rename = "occurredAtMs", alias = "occurred_at_ms")]
    pub occurred_at_ms: i64,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EarthquakesResponse {
    pub rows: Vec<QuakeRow>,
    #[serde(rename = "maxMag")]
    pub max_mag: f64,
    pub total: usize,
    #[serde(rename = "assembledAtMs")]
    pub assembled_at_ms: i64,
    pub stale: bool,
}

#[derive(Debug, Deserialize)]
struct Snap {
    #[serde(default)]
    rows: Vec<QuakeRow>,
    #[serde(default, alias = "assembledAtMs")]
    assembled_at_ms: i64,
}

pub async fn handler(
    State(state): State<AppState>,
) -> Result<Json<EarthquakesResponse>, HandlerError> {
    let raw = get_cached_json::<Value>(&state.pool, CACHE_KEY)
        .await
        .map_err(|e| HandlerError::Cache(e.to_string()))?;
    let (snap, stale) = decode_required::<Snap>(raw)?;
    let total = snap.rows.len();
    let max_mag = snap.rows.iter().map(|r| r.mag).fold(0.0_f64, f64::max);
    Ok(Json(EarthquakesResponse {
        rows: snap.rows,
        max_mag,
        total,
        assembled_at_ms: snap.assembled_at_ms,
        stale,
    }))
}

#[cfg(test)]
#[allow(clippy::panic, clippy::unwrap_used, clippy::expect_used)]
mod tests {
    use super::*;
    use crate::climate::v1::EARTHQUAKES_PATH;
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use pellucid_cache::set_cached_json;
    use pellucid_core::Envelope;
    use tower::ServiceExt;

    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        let state = AppState::for_tests_async().await.unwrap();
        let pool = state.pool.clone();
        let app = axum::Router::new().route(
            EARTHQUAKES_PATH,
            axum::routing::get(handler).with_state(state),
        );
        (app, pool)
    }

    #[tokio::test]
    async fn returns_503_when_cache_empty() {
        let (app, _) = migrated().await;
        let resp = app
            .oneshot(Request::builder().uri(EARTHQUAKES_PATH).body(Body::empty()).unwrap())
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::SERVICE_UNAVAILABLE);
    }

    #[tokio::test]
    async fn computes_max_mag() {
        let (app, pool) = migrated().await;
        let snap = serde_json::json!({
            "rows": [
                { "id": "us1", "place": "Off CA", "mag": 4.2, "depth": 12.0, "lat": 36.0, "lon": -120.0, "occurred_at_ms": 1_700_000_000_000_i64 },
                { "id": "us2", "place": "Japan", "mag": 6.1, "depth": 50.0, "lat": 35.0, "lon": 140.0, "occurred_at_ms": 1_700_001_000_000_i64 },
            ],
            "assembled_at_ms": 1_700_000_000_000_i64,
        });
        set_cached_json(&pool, CACHE_KEY, &Envelope::new(snap), 60_000).await.unwrap();
        let resp = app.oneshot(Request::builder().uri(EARTHQUAKES_PATH).body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = axum::body::to_bytes(resp.into_body(), 1_000_000).await.unwrap();
        let parsed: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(parsed.pointer("/maxMag").and_then(Value::as_f64), Some(6.1));
        assert_eq!(parsed.pointer("/total").and_then(Value::as_u64), Some(2));
    }
}
