//! `GET /api/energy/v1/hormuz` — pure cache reader for the Strait
//! of Hormuz transit-flow snapshot.
...
pub struct HormuzRow {
    pub product: String,
    #[serde(rename = "mbPerDay", alias = "mb_per_day")]
    pub mb_per_day: f64,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct HormuzResponse {
    pub rows: Vec<HormuzRow>,
    #[serde(rename = "totalMbPerDay")]
...
struct Snap {
    #[serde(default)]
    rows: Vec<HormuzRow>,
    #[serde(default, alias = "totalMbPerDay")]
    total_mb_per_day: f64,
    #[serde(default)]
    period: String,
...
pub async fn handler(
    ...
    Ok(Json(HormuzResponse {
        rows: snap.rows,
        total_mb_per_day: snap.total_mb_per_day,
        period: snap.period,
...
mod tests {
    use super::*;
    use crate::energy::v1::HORMUZ_PATH;
    use axum::body::Body;
    ...
    async fn returns_camelcase_payload() {
        ...
        let snap = serde_json::json!({
            "rows": [
                { "product": "crude", "mb_per_day": 14.0, "share_pct": 70.0 },
                { "product": "lng",   "mb_per_day": 4.0,  "share_pct": 20.0 },
            ],
            "total_mb_per_day": 20.0,
            "period": "2026-04",
