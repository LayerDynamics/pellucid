//! `GET /api/energy/v1/renewable` — pure reader for the IRENA
//! renewable-capacity mix snapshot.

use axum::extract::State;
use axum::Json;
use serde::{Deserialize, Serialize};
use serde_json::Value;

use pellucid_cache::get_cached_json;

use crate::economic::v1::shared::{decode_required, HandlerError};
use crate::state::AppState;

pub const CACHE_KEY: &str = "energy:renewable-mix:current:v1";

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct RenewableSourceRow {
    pub source: String,
    #[serde(rename = "capacityGw", alias = "capacity_gw")]
    pub capacity_gw: f64,
    #[serde(rename = "yoyPct", alias = "yoy_pct")]
    pub yoy_pct: f64,
    #[serde(rename = "sharePct", alias = "share_pct")]
    pub share_pct: f64,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct RenewableResponse {
    pub rows: Vec<RenewableSourceRow>,
    #[serde(rename = "totalCapacityGw")]
    pub total_capacity_gw: f64,
    pub period: String,
    #[serde(rename = "assembledAtMs")]
    pub assembled_at_ms: i64,
    pub stale: bool,
}

#[derive(Debug, Deserialize)]
struct Snap {
    #[serde(default)]
    rows: Vec<RenewableSourceRow>,
    #[serde(default, alias = "totalCapacityGw")]
    total_capacity_gw: f64,
    #[serde(default)]
    period: String,
    #[serde(default, alias = "assembledAtMs")]
    assembled_at_ms: i64,
}

pub async fn handler(
    State(state): State<AppState>,
) -> Result<Json<RenewableResponse>, HandlerError> {
    let raw = get_cached_json::<Value>(&state.pool, CACHE_KEY)
        .await
        .map_err(|e| HandlerError::Cache(e.to_string()))?;
    let (snap, stale) = decode_required::<Snap>(raw)?;
    Ok(Json(RenewableResponse {
        rows: snap.rows,
        total_capacity_gw: snap.total_capacity_gw,
        period: snap.period,
        assembled_at_ms: snap.assembled_at_ms,
        stale,
    }))
}

#[cfg(test)]
#[allow(clippy::panic, clippy::unwrap_used, clippy::expect_used)]
mod tests {
    use super::*;
    use crate::energy::v1::RENEWABLE_PATH;
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use pellucid_cache::set_cached_json;
    use pellucid_core::Envelope;
    use tower::ServiceExt;

    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        let state = AppState::for_tests_async().await.unwrap();
        let pool = state.pool.clone();
        let app = axum::Router::new().route(
            RENEWABLE_PATH,
            axum::routing::get(handler).with_state(state),
        );
        (app, pool)
    }

    #[tokio::test]
    async fn returns_503_when_cache_empty() {
        let (app, _) = migrated().await;
        let resp = app
            .oneshot(
                Request::builder()
                    .uri(RENEWABLE_PATH)
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::SERVICE_UNAVAILABLE);
    }

    #[tokio::test]
    async fn returns_camelcase_payload() {
        let (app, pool) = migrated().await;
        let snap = serde_json::json!({
            "rows": [
                { "source": "solar", "capacity_gw": 1500.0, "yoy_pct": 22.0, "share_pct": 39.0 },
            ],
            "total_capacity_gw": 3800.0,
            "period": "2026-Q1",
            "assembled_at_ms": 1_700_000_000_000_i64,
        });
        set_cached_json(&pool, CACHE_KEY, &Envelope::new(snap), 60_000)
            .await
            .unwrap();
        let resp = app
            .oneshot(
                Request::builder()
                    .uri(RENEWABLE_PATH)
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = axum::body::to_bytes(resp.into_body(), 1_000_000)
            .await
            .unwrap();
        let parsed: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(
            parsed.pointer("/totalCapacityGw").and_then(Value::as_f64),
            Some(3800.0)
        );
        assert_eq!(
            parsed.pointer("/rows/0/yoyPct").and_then(Value::as_f64),
            Some(22.0)
        );
    }
}
