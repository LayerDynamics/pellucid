//! `GET /api/forecast/v1/prediction-markets` — pure reader of
//! Polymarket scenario state.
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct PredictionMarketsResponse {
...
pub async fn handler(
...
) -> Result<Json<PredictionMarketsResponse>, HandlerError> {
    ...
    Ok(Json(PredictionMarketsResponse {
        rows: snap.rows,
        total,
...
mod tests {
    use super::*;
    use crate::forecast::v1::PREDICTION_MARKETS_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(PREDICTION_MARKETS_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(PREDICTION_MARKETS_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn returns_total() {
        ...
        let snap = serde_json::json!({
            "rows": [{ "id": "p1", "question": "Q", "yes_price": 0.7, "volume_usd": 12000.0, "category": "geo" }],
