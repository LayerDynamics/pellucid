...
pub async fn handler(
    ...
    let top_row = snap
        .rows
        .iter()
        .max_by(|a, b| a.yes_price.partial_cmp(&b.yes_price).unwrap_or(std::cmp::Ordering::Equal))
        .cloned();
...
mod tests {
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(SCENARIO_STATE_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(SCENARIO_STATE_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn computes_weighted_avg_and_top() {
        ...
        let avg = parsed.pointer("/weightedAvgYes").and_then(Value::as_f64).unwrap();
        assert!((avg - 0.87).abs() < 0.001);
        assert_eq!(parsed.pointer("/topRow/id").and_then(Value::as_str), Some("b"));
    }

    #[test]
    fn weighted_avg_zero_volume_returns_zero() {
        let rows = vec![StateRow {
            id: "x".into(), question: "q".into(),
            yes_price: 0.5, volume_usd: 0.0, category: "g".into(),
        }];
