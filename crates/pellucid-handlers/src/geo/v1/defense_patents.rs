//! `GET /api/geo/v1/defense-patents` — pure reader for defense
//! patent-trends snapshot.
...
mod tests {
    use super::*;
    use crate::geo::v1::DEFENSE_PATENTS_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(DEFENSE_PATENTS_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(DEFENSE_PATENTS_PATH).body(Body::empty()).unwrap()).await.unwrap();
