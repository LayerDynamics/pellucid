//! `GET /api/geo/v1/escalation-correlation` — composer that
//! correlates thermal anomalies with UCDP events by zone (joining
//! `zone` against `country`). One row per zone that appears in
//! BOTH thermal feed AND UCDP rows; sorted by descending
//! escalation score.
...
use super::{thermal_escalation, ucdp_events};
...
mod tests {
    use super::*;
    use crate::geo::v1::ESCALATION_CORRELATION_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(ESCALATION_CORRELATION_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_both_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(ESCALATION_CORRELATION_PATH).body(Body::empty()).unwrap()).await.unwrap();
