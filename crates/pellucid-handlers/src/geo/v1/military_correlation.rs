//! `GET /api/geo/v1/military-correlation` — composer that
...
mod tests {
    use super::*;
    use crate::geo::v1::MILITARY_CORRELATION_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(MILITARY_CORRELATION_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_posture_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(MILITARY_CORRELATION_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn correlates_when_posture_seeded() {
        ...
        set_cached_json(&pool, strategic_posture::CACHE_KEY, &Envelope::new(posture), 60_000).await.unwrap();
        let resp = app.oneshot(Request::builder().uri(MILITARY_CORRELATION_PATH).body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = axum::body::to_bytes(resp.into_body(), 1_000_000).await.unwrap();
        let parsed: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(parsed.pointer("/total").and_then(Value::as_u64), Some(1));
        assert_eq!(parsed.pointer("/rows/0/theater").and_then(Value::as_str), Some("EUCOM"));
