//! `GET /api/geo/v1/sanctions-pressure` — pure reader for
//! sanctions recent-additions slot.
...
mod tests {
    use super::*;
    use crate::geo::v1::SANCTIONS_PRESSURE_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(SANCTIONS_PRESSURE_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(SANCTIONS_PRESSURE_PATH).body(Body::empty()).unwrap()).await.unwrap();
