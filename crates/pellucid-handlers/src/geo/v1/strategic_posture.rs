//! `GET /api/geo/v1/strategic-posture` — pure reader for theater
//! posture snapshot.
...
mod tests {
    use super::*;
    use crate::geo::v1::STRATEGIC_POSTURE_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(STRATEGIC_POSTURE_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(STRATEGIC_POSTURE_PATH).body(Body::empty()).unwrap()).await.unwrap();
