//! `GET /api/geo/v1/strategic-risk` — composer that blends UCDP
//! events + theater posture + sanctions pressure into a 0..=100
//! strategic-risk score. Pure cache reader.
...
use super::{strategic_posture, ucdp_events};
...
pub fn level_from_score(score: u32) -> String {
    if score >= 60 { "high".into() } else if score >= 30 { "elevated".into() } else { "low".into() }
}
...
pub async fn handler(
    ...
    let high_readiness = p
        .as_ref()
        .map(|p| p.rows.iter().filter(|r| matches!(r.readiness.as_str(), "C-1" | "DEFCON-1" | "DEFCON-2" | "high")).count());
    let sanctions = s.as_ref().map(|s| s.rows.len());

    if fatalities.is_none() && high_readiness.is_none() && sanctions.is_none() {
        return Err(HandlerError::Outage { retry_after_secs: DEFAULT_RETRY_AFTER_SECS });
    }
    let score = score_from_signals(fatalities, high_readiness, sanctions);
    let level = level_from_score(score);
    let mut parts: Vec<String> = Vec::new();
    if let Some(f) = fatalities { parts.push(format!("UCDP 24h: {f} fatalities")); }
    if let Some(n) = high_readiness { parts.push(format!("{n} high-readiness theaters")); }
    if let Some(n) = sanctions { parts.push(format!("{n} sanctions in 24h")); }
...
mod tests {
    use super::*;
    use crate::geo::v1::STRATEGIC_RISK_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(STRATEGIC_RISK_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_all_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(STRATEGIC_RISK_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn computes_low_score_when_quiet() {
        ...
        set_cached_json(&pool, ucdp_events::CACHE_KEY, &Envelope::new(serde_json::json!({"rows":[],"assembled_at_ms":0})), 60_000).await.unwrap();
        let resp = app.oneshot(Request::builder().uri(STRATEGIC_RISK_PATH).body(Body::empty()).unwrap()).await.unwrap();
