//! `GET /api/geo/v1/supply-chain` — composer over supply-chain
//! stress-index + port-congestion + chokepoint-status (3 real
//! cache slots seeded by T3.8).
...
fn stress_tone(v: f64) -> &'static str {
    if v >= 0.6 { "negative" } else if v >= 0.3 { "neutral" } else { "positive" }
}
...
pub async fn handler(
    ...
    if let Some(p) = port {
        ...
        tiles.push(SupplyChainTile {
            ...
            value: n.to_string(),
            tone: if n == 0 { "positive".into() } else if n <= 3 { "neutral".into() } else { "negative".into() },
        });
    }
    if let Some(c) = choke {
        let degraded = c.rows.iter().filter(|r| !matches!(r.status.to_ascii_lowercase().as_str(), "open" | "normal" | "ok")).count();
        tiles.push(SupplyChainTile {
...
mod tests {
    use super::*;
    use crate::geo::v1::SUPPLY_CHAIN_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(SUPPLY_CHAIN_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_all_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(SUPPLY_CHAIN_PATH).body(Body::empty()).unwrap()).await.unwrap();
