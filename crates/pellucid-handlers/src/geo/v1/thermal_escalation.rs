//! `GET /api/geo/v1/thermal-escalation` — pure reader for thermal
//! anomaly feed in conflict zones.

use axum::extract::State;
use axum::Json;
use serde::{Deserialize, Serialize};
use serde_json::Value;

use pellucid_cache::get_cached_json;

use crate::economic::v1::shared::{decode_required, HandlerError};
use crate::state::AppState;

pub const CACHE_KEY: &str = "thermal:anomaly-feed:current:v1";

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ThermalRow {
    pub id: String,
    pub lat: f64,
    pub lon: f64,
    #[serde(rename = "brightnessK", alias = "brightness_k")]
    pub brightness_k: f64,
    pub confidence: u32,
    pub zone: String,
    #[serde(rename = "acquiredAt", alias = "acquired_at")]
    pub acquired_at: String,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ThermalEscalationResponse {
    pub rows: Vec<ThermalRow>,
    #[serde(rename = "highConfidenceCount")]
    pub high_confidence_count: usize,
    pub total: usize,
    #[serde(rename = "assembledAtMs")]
    pub assembled_at_ms: i64,
    pub stale: bool,
}

#[derive(Debug, Deserialize)]
struct Snap {
    #[serde(default)]
    rows: Vec<ThermalRow>,
    #[serde(default, alias = "assembledAtMs")]
    assembled_at_ms: i64,
}

pub async fn handler(
    State(state): State<AppState>,
) -> Result<Json<ThermalEscalationResponse>, HandlerError> {
    let raw = get_cached_json::<Value>(&state.pool, CACHE_KEY)
        .await
        .map_err(|e| HandlerError::Cache(e.to_string()))?;
    let (snap, stale) = decode_required::<Snap>(raw)?;
    let total = snap.rows.len();
    let high_confidence_count = snap.rows.iter().filter(|r| r.confidence >= 80).count();
    Ok(Json(ThermalEscalationResponse {
        rows: snap.rows,
        high_confidence_count,
        total,
        assembled_at_ms: snap.assembled_at_ms,
        stale,
    }))
}

#[cfg(test)]
#[allow(clippy::panic, clippy::unwrap_used, clippy::expect_used)]
mod tests {
    use super::*;
    use crate::geo::v1::THERMAL_ESCALATION_PATH;
    use axum::body::Body;
    use axum::http::{Request, StatusCode};
    use pellucid_cache::set_cached_json;
    use pellucid_core::Envelope;
    use tower::ServiceExt;

    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        let state = AppState::for_tests_async().await.unwrap();
        let pool = state.pool.clone();
        let app = axum::Router::new().route(THERMAL_ESCALATION_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }

    #[tokio::test]
    async fn returns_503_when_empty() {
        let (app, _) = migrated().await;
        let resp = app.oneshot(Request::builder().uri(THERMAL_ESCALATION_PATH).body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::SERVICE_UNAVAILABLE);
    }

    #[tokio::test]
    async fn counts_high_confidence() {
        let (app, pool) = migrated().await;
        let snap = serde_json::json!({
            "rows": [
                { "id": "a", "lat": 50.0, "lon": 30.0, "brightness_k": 410.0, "confidence": 90, "zone": "Ukraine", "acquired_at": "2026-04-29T12:00:00Z" },
                { "id": "b", "lat": 31.0, "lon": 34.0, "brightness_k": 360.0, "confidence": 60, "zone": "Gaza",    "acquired_at": "2026-04-29T13:00:00Z" },
            ],
            "assembled_at_ms": 1_700_000_000_000_i64,
        });
        set_cached_json(&pool, CACHE_KEY, &Envelope::new(snap), 60_000).await.unwrap();
        let resp = app.oneshot(Request::builder().uri(THERMAL_ESCALATION_PATH).body(Body::empty()).unwrap()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = axum::body::to_bytes(resp.into_body(), 1_000_000).await.unwrap();
        let parsed: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(parsed.pointer("/highConfidenceCount").and_then(Value::as_u64), Some(1));
        assert_eq!(parsed.pointer("/rows/0/brightnessK").and_then(Value::as_f64), Some(410.0));
    }
}
