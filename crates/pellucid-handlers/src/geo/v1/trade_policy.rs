//! `GET /api/geo/v1/trade-policy` — pure reader for tariff
//! alerts.
...
mod tests {
    use super::*;
    use crate::geo::v1::TRADE_POLICY_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(TRADE_POLICY_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(TRADE_POLICY_PATH).body(Body::empty()).unwrap()).await.unwrap();
