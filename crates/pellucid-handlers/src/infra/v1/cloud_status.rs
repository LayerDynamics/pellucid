...
mod tests {
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(CLOUD_STATUS_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(CLOUD_STATUS_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn counts_incidents() {
        ...
        assert_eq!(parsed.pointer("/incidentCount").and_then(Value::as_u64), Some(1));
