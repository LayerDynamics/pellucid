//! `GET /api/infra/v1/cyber-incidents` — pure reader of the
//! cyber incident feed (24 h).
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct CyberIncidentsResponse {
...
pub async fn handler(
...
) -> Result<Json<CyberIncidentsResponse>, HandlerError> {
    ...
    Ok(Json(CyberIncidentsResponse {
        rows: snap.rows,
        total,
...
mod tests {
    use super::*;
    use crate::infra::v1::CYBER_INCIDENTS_PATH;
    use axum::body::Body;
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app = axum::Router::new().route(CYBER_INCIDENTS_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_503_when_empty() {
        ...
        let resp = app.oneshot(Request::builder().uri(CYBER_INCIDENTS_PATH).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn returns_total() {
        ...
        let snap = serde_json::json!({
            "rows": [{ "id": "i1", "title": "Ransomware: ACME Co.", "severity": "high", "source": "BleepingComputer", "published_at_ms": 1_700_000_000_000_i64 }],
