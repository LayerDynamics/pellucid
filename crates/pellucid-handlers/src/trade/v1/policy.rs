...
mod tests {
    ...
    async fn migrated() -> (axum::Router, pellucid_db::Pool) {
        ...
        let app =
            axum::Router::new().route(POLICY_PATH, axum::routing::get(handler).with_state(state));
        (app, pool)
    }
    ...
    async fn returns_camelcase() {
        ...
        assert_eq!(
            parsed.pointer("/totalRateDeltaPp").and_then(Value::as_f64),
            Some(25.0)
        );
        assert_eq!(
            parsed.pointer("/rows/0/hsCode").and_then(Value::as_str),
            Some("8542")
        );
    }
}
