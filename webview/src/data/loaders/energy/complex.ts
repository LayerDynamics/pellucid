/** Energy complex composer loader. */

export interface ComplexTile {
  code: string;
  label: string;
  value: string;
  subline?: string;
...
export type ComplexOutcome =
  | { kind: "ready"; response: ComplexResponse }
...
export async function loadEnergyComplex(opts: LoadOptions = {}): Promise<ComplexOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/energy/v1/complex`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as ComplexResponse }; }
