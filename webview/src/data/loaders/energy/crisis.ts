/** Energy crisis composer loader. */

export interface CrisisResponse {
  level: "low" | "elevated" | "high" | string;
  score: number;
  headline: string;
  rationale: string;
  gasStoragePct?: number;
  sprMb?: number;
  sprDeltaMb?: number;
...
export type CrisisOutcome =
  | { kind: "ready"; response: CrisisResponse }
...
export async function loadEnergyCrisis(opts: LoadOptions = {}): Promise<CrisisOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/energy/v1/crisis`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as CrisisResponse }; }
