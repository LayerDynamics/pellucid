/** Gold intelligence loader. */

export interface GoldEtfRow {
  ticker: string;
  region: string;
  netFlowMillionUsd: number;
  tonneHoldings: number;
}

export interface GoldIntelligenceResponse {
  rows: GoldEtfRow[];
  totalNetFlowMillionUsd: number;
  totalTonneHoldings: number;
  assembledAtMs: number;
  stale: boolean;
}

export type GoldIntelOutcome =
  | { kind: "ready"; response: GoldIntelligenceResponse }
...
export async function loadGoldIntelligence(opts: LoadOptions = {}): Promise<GoldIntelOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/commodities/v1/gold-intelligence`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as GoldIntelligenceResponse }; }
