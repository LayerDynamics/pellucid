/** Oil inventories loader. */

export interface OilStocksRow {
  product: string;
  period: string;
  valueMb: number;
  wowDeltaMb: number;
...
export interface OilInventoriesResponse {
  rows: OilStocksRow[];
  assembledAtMs: number;
  stale: boolean;
}

export type OilInventoriesOutcome =
  | { kind: "ready"; response: OilInventoriesResponse }
...
export async function loadOilInventories(opts: LoadOptions = {}): Promise<OilInventoriesOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/energy/v1/oil-inventories`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as OilInventoriesResponse }; }
