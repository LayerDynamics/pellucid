/** Renewable mix loader. */

export interface RenewableSourceRow {
  source: string;
  capacityGw: number;
  yoyPct: number;
  sharePct: number;
...
export type RenewableOutcome =
  | { kind: "ready"; response: RenewableResponse }
...
export async function loadRenewableMix(opts: LoadOptions = {}): Promise<RenewableOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/energy/v1/renewable`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as RenewableResponse }; }
