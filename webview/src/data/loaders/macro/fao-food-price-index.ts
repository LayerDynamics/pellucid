/** FAO food price index loader. */

export interface FaoObservation {
  period: string;
  composite: number;
  subindices: Array<[string, number]>;
}

export interface FaoResponse {
  latest: FaoObservation;
  history: FaoObservation[];
  yoyPct: number;
  assembledAtMs: number;
  stale: boolean;
}

export type FaoOutcome =
  | { kind: "ready"; response: FaoResponse }
...
export async function loadFaoFoodPriceIndex(opts: LoadOptions = {}): Promise<FaoOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/fao-food-price-index`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as FaoResponse }; }
