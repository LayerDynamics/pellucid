/** FSI loader — wraps `/api/economic/v1/financial-stress-index`. */
...
export interface FsiResponse {
  seriesCode: string;
  latest: FsiObservation;
  prior?: FsiObservation;
  history: FsiObservation[];
  assembledAtMs: number;
  stale: boolean;
}

export type FsiOutcome =
  | { kind: "ready"; response: FsiResponse }
...
export async function loadFinancialStress(opts: LoadOptions = {}): Promise<FsiOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/financial-stress-index`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as FsiResponse }; }
