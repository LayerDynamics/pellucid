/** Gulf economies loader. */

export interface GulfCountryRow {
  iso: string;
  country: string;
...
export interface GulfResponse {
  rows: GulfCountryRow[];
  assembledAtMs: number;
  stale: boolean;
}

export type GulfOutcome =
  | { kind: "ready"; response: GulfResponse }
...
export async function loadGulfEconomies(opts: LoadOptions = {}): Promise<GulfOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/gulf-economies`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as GulfResponse }; }
