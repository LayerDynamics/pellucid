/** Macro signals loader. */

export type SignalDirection = "rising" | "falling" | "flat";
...
export type MacroSignalsOutcome =
  | { kind: "ready"; response: MacroSignalsResponse }
...
export async function loadMacroSignals(opts: LoadOptions = {}): Promise<MacroSignalsOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/macro-signals`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as MacroSignalsResponse }; }
