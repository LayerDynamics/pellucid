...
export interface MacroTilesResponse {
  tiles: MacroTile[];
  assembledAtMs: number;
  stale: boolean;
}

export type MacroTilesOutcome =
  | { kind: "ready"; response: MacroTilesResponse }
...
export async function loadMacroTiles(opts: LoadOptions = {}): Promise<MacroTilesOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/macro-tiles`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as MacroTilesResponse }; }
