...
export interface NationalDebtResponse {
  ...
  qoqDeltaBillionUsd: number;
  history: DebtObservation[];
  assembledAtMs: number;
  stale: boolean;
}

export type NationalDebtOutcome =
  | { kind: "ready"; response: NationalDebtResponse }
...
export async function loadNationalDebt(opts: LoadOptions = {}): Promise<NationalDebtOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/national-debt`;
  ...
  if (httpStatus === 200) {
    try { return { kind: "ready", response: (await resp.json()) as NationalDebtResponse }; }
