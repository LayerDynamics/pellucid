/** Economic snapshot loader — wraps `/api/economic/v1/snapshot`. */
...
export type SnapshotOutcome =
  | { kind: "ready"; response: SnapshotResponse }
...
export async function loadEconomicSnapshot(opts: LoadOptions = {}): Promise<SnapshotOutcome> {
  const url = `${opts.baseUrl ?? ""}/api/economic/v1/snapshot`;
  ...
  if (httpStatus === 200) {
    try {
      return { kind: "ready", response: (await resp.json()) as SnapshotResponse };
    } catch (e) {
      return { kind: "error", code: "unknown", message: `parse: ${e instanceof Error ? e.message : String(e)}`, httpStatus, retryAfterSecs };
    }
  }
