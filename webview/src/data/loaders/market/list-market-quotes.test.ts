...
describe("loadMarketQuotes", () => {
  test("200 with valid body → kind=ready", async () => {
    ...
    const out = await loadMarketQuotes({}, { fetchImpl });
    expect(out.kind).toBe("ready");
    if (out.kind === "ready") expect(out.response).toEqual(SAMPLE);
    expect(calls[0]).toBe("/api/market/v1/list-market-quotes");
  });

  test("encodes ?limit + ?symbols (csv) into the query string", async () => {
    const seen: string[] = [];
    const fetchImpl: typeof fetch = async (input) => {
    ...
    await loadMarketQuotes(
      { limit: 25, symbols: ["SPY", " QQQ", "DIA "] },
      { fetchImpl },
    );
    const url = seen[0] ?? "";
    expect(url).toContain("limit=25");
    expect(url).toContain("symbols=SPY%2CQQQ%2CDIA");
  });

  test("trims + drops empty symbol entries", async () => {
    const seen: string[] = [];
    const fetchImpl: typeof fetch = async (input) => {
      seen.push(typeof input === "string" ? input : (input as URL).toString());
      return jsonResponse(200, SAMPLE);
    ...
    expect(seen[0] ?? "").toContain("symbols=SPY");
    seen.length = 0;
    await loadMarketQuotes({ symbols: ["", " "] }, { fetchImpl });
    expect(seen[0] ?? "").toBe("/api/market/v1/list-market-quotes");
  });
  ...
  test("clamps non-finite or fractional limit to floor (>=1)", async () => {
    ...
    await loadMarketQuotes({ limit: 0 }, { fetchImpl });
  ...
  test("503 with bootstrap_upstream_empty body + retry-after header", async () => {
    ...
    const out: ListMarketQuotesOutcome = await loadMarketQuotes(
      {},
      { fetchImpl },
    );
    expect(out.kind).toBe("error");
  ...
  test("bearer token forwarded in Authorization header", async () => {
    ...
    await loadMarketQuotes({}, { fetchImpl, bearerToken: "tok-1" });
  ...
  test("baseUrl prefixes the request URL", async () => {
    ...
    await loadMarketQuotes({}, { fetchImpl, baseUrl: "https://api.example" });
    expect(seen[0]).toBe(
      "https://api.example/api/market/v1/list-market-quotes",
    );
  });
  ...
  test("AbortSignal is propagated to fetch", async () => {
    ...
    await loadMarketQuotes({}, { fetchImpl, signal: ctrl.signal });
