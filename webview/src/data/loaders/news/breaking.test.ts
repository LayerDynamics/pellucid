...
import {
  ...
  type GetBreakingResponse,
} from "./breaking";

const SAMPLE: GetBreakingResponse = {
  articles: [
    {
      id: "b1",
      title: "Wildfire alert",
      source: "AP",
      publishedAtMs: 1_700_000_200_000,
      severity: "critical",
    },
  ],
  total: 1,
...
describe("loadBreakingNews", () => {
  test("200 with valid body → kind=ready", async () => {
    ...
    const out = await loadBreakingNews({}, { fetchImpl });
    expect(out.kind).toBe("ready");
    if (out.kind === "ready") expect(out.response).toEqual(SAMPLE);
    expect(calls[0]).toBe("/api/news/v1/get-breaking");
  });

  test("encodes ?limit + ?severity + ?sinceMs into the query string", async () => {
    const seen: string[] = [];
    const fetchImpl: typeof fetch = async (input) => {
    ...
    await loadBreakingNews(
      { limit: 3, severity: "critical", sinceMs: 1_700_000_000_000 },
      { fetchImpl },
    );
    const url = seen[0] ?? "";
    expect(url).toContain("limit=3");
    expect(url).toContain("severity=critical");
    expect(url).toContain("sinceMs=1700000000000");
  });
  ...
  test("clamps non-finite or fractional limit to floor (>=1)", async () => {
    ...
    await loadBreakingNews({ limit: 0 }, { fetchImpl });
    const url = seen[0] ?? "";
    expect(url).toContain("limit=1");
  });

  test("non-finite sinceMs is dropped (not encoded as 'NaN')", async () => {
    const seen: string[] = [];
    const fetchImpl: typeof fetch = async (input) => {
      seen.push(typeof input === "string" ? input : (input as URL).toString());
      return jsonResponse(200, SAMPLE);
    };
    await loadBreakingNews({ sinceMs: Number.NaN }, { fetchImpl });
    const url = seen[0] ?? "";
    expect(url).toBe("/api/news/v1/get-breaking");
  });
  ...
  test("503 with bootstrap_upstream_empty body + retry-after header", async () => {
    ...
    const out: GetBreakingOutcome = await loadBreakingNews({}, { fetchImpl });
    expect(out.kind).toBe("error");
    if (out.kind === "error") {
  ...
  test("bearer token forwarded in Authorization header", async () => {
    ...
    await loadBreakingNews({}, { fetchImpl, bearerToken: "tok-123" });
  ...
  test("baseUrl prefixes the request URL", async () => {
    ...
    await loadBreakingNews({}, { fetchImpl, baseUrl: "https://api.example" });
    expect(seen[0]).toBe("https://api.example/api/news/v1/get-breaking");
  });
  ...
  test("AbortSignal is propagated to fetch", async () => {
    ...
    await loadBreakingNews({}, { fetchImpl, signal: ctrl.signal });
