/**
 * Energy / commodities panel-family registry. Mirrors
 * panels/macro shape — same registerPanel / listPanels surface.
 */
...
export function registerPanel(descriptor: PanelDescriptor): void {
  if (REGISTERED_IDS.has(descriptor.id)) {
    throw new Error(
      `panels/energy: duplicate panel id ${JSON.stringify(descriptor.id)}`,
    );
  }
...
/** Test-only — clear the registry between describe blocks. */
